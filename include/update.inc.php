...
// Version 13.6.04
if (
	$this->getData(['core', 'dataVersion']) < 13604
) {
	if (file_exists('core/class/layout.class.php')) {
		unlink('core/class/layout.class.php');
	}
	$this->setData(['core', 'dataVersion', 13604]);
}
// Version 13.6.05
if (
	$this->getData(['core', 'dataVersion']) < 13605
) {
	// Renomme les chemins en supprimant les / au début des chemins de profil
	$roles = array(1, 2);
	foreach ($roles as $roleId => $roleValue) {
		foreach ($this->getData(['profil', $roleValue]) as $profilId => $profilValue) {
			$path = $this->getData(['profil', $roleValue, $profilId, 'folder', 'path']);
			if (is_null($path) === false) {
				$data = ltrim($path, '/');
				$this->setData(['profil', $roleValue, $profilId, 'folder', 'path', $data], false);
			}
		}
	}
	$this->saveDb('profil');

	// Supprime le module folder
	if (is_dir('core/module/folder')) {
		$this->deleteDir('core/module/folder');
	}
	// Parcourir les pages et modifier les modules folder en resource
	$hierarchy = $this->getData(['page']);
	foreach ($hierarchy as $parentKey => $parentValue) {
		if ($this->getData(['page', $parentKey, 'moduleId']) === 'folder') {
			$this->setData(['page', $parentKey, 'moduleId', 'resource'], false);
		}
	}
	$this->saveDb('page');

	$this->setData(['core', 'dataVersion', 13605]);
}

// Version 13.11.00
if (
	$this->getData(['core', 'dataVersion']) < 131100
) {
	// Liste des dossiers de plugin TinyMCE à supprimer
	$pluginsToDelete = [
			'colorîcker',
			'contextmenu',
			'textcolor',
	...
	// Initialisation
	$basePath = 'core/vendor/tinymce/plugins/';
	...
	$this->setData(['core', 'dataVersion', 131100]);
}


// Version 14.00.00 - Installation configuration database depuis defaultData
if ($this->getData(['core', 'dataVersion']) < 140000) {
	
	// Installation de la configuration database depuis defaultData
	
	// 1. Charger defaultData pour obtenir la configuration database
	require_once('core/module/install/ressource/defaultdata.php');
	$databaseConfig = init::$defaultData['config']['database'] ?? [];
	
	// 2. Installer la configuration database dans config.json
	$this->setData(['config', 'database', $databaseConfig]);
	
	// 3. Nettoyage du cache Dot si présent
	if (class_exists('\Prowebcraft\Dot')) {
		\Prowebcraft\Dot::clearKeyCache();
	}
	...
	$migrationReport = [
		'version' => '14.00.00',
		'timestamp' => time(),
		'database_config_installed' => true,
		'databases_configured' => array_keys($databaseConfig),
		'features_enabled' => [
			'granular_database_config' => true,
			'dot_cache_cleanup' => true
		]
	];
	
	// 5. Sauvegarder le rapport de migration pour diagnostic
	$this->saveLog('ZwiiCMS Migration 14.00.00: Database configuration installed from defaultData');
	$this->saveLog('Migration report: ' . json_encode($migrationReport));
	
	// Mettre à jour la version des données
	$this->setData(['core', 'dataVersion', 140000]);
}
